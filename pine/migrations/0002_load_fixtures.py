...
def load_fixtures(apps, schema_editor):
    ...
    auth_arr = [
        auth(username='01032080403', password='pbkdf2_sha256$12000$tGQyY9GZEAdK$XRVF89JR08p1nDMZHtW7oboFR/QBu+HPGjHgc+ViY50='),
        auth(username='01098590530', password='pbkdf2_sha256$12000$5j1JgtXTctvi$7kjTpkJxaN9QNv284znZk107r+EBi8kQRCL7dl6qi8U='),
        auth(username='01087537711', password='pbkdf2_sha256$12000$aLZWPLqrhhog$IrRvZoolnkGkf2ETEkbHIGlsStednB/nWJAOR1bhIJM='),
        auth(username='01021101783', password='pbkdf2_sha256$12000$GfKcTLnKbbxH$vFeD217URuUd6UoEBpyO51HB5eMeCfpGCGBqM01pA7Q='),
        auth(username='01040099179', password='pbkdf2_sha256$12000$IK3q5GpLBBFD$eoHtTaD/7JmueXZeNJLRKEPOBlcDuKEVkQR+3age/JE='),
        auth(username='01089607165', password='pbkdf2_sha256$12000$AKterSZxVwMi$wAqun/ijfrVwoNijuAMXqfUlERVhKb06NNkjosYMO3M='),
        auth(username='01037585989', password='pbkdf2_sha256$12000$uqZSeowDeZ4a$mh/q20d3+x9RmvRq+crA0TmY90C4afWiyEMHmTwZj+Y='),
        auth(username='01020624493', password='pbkdf2_sha256$12000$NpHE2KM6alSU$YQzeciGQfoJKu+jy0x2PEnJzayXgiBlNGxq5MxOj/mk='),
        auth(username='01020863441', password='pbkdf2_sha256$12000$iqBQweNoB59u$aZJm2mYg924YFo/gcgOZAD2EQUIivf8nAvK3haeVVco='),
        auth(username='01092696960', password='pbkdf2_sha256$12000$kFW0pCZ4trad$brBG4Niz7dGLQ6lFwY7uuh8G32N8ZdHw/BBmtAN8JKk='),
    ]
    auth.objects.using(db_alias).bulk_create(auth_arr)
    ...
    phones.objects.using(db_alias).bulk_create(phones_arr)

    users = apps.get_model('pine', 'Users')
    users_arr = [
        users(account=auth_arr[0], phone=phones_arr[0],
              device='none',
              push_id=''),
        users(account=auth_arr[1], phone=phones_arr[1],
              # friend_phones=[
              #     phones_arr[1],
              #     phones_arr[2],
              #     phones_arr[3],
              ...
              device='ios',
              push_id=''),
        users(account=auth_arr[2], phone=phones_arr[2],
              # friend_phones=[
              #     phones_arr[0],
              #     phones_arr[1],
              #     phones_arr[2],
              ...
              device='none',
              push_id=''),
        users(account=auth_arr[3], phone=phones_arr[3],
              # friend_phones=[
              #     phones_arr[0],
              #     phones_arr[1],
              #     phones_arr[2],
              ...
              device='none',
              push_id=''),
        users(account=auth_arr[4], phone=phones_arr[4],
              # friend_phones=[1, 7, 8],
              # friends=[1, 6],
              # followings=[7, 8],
              # blocks=[],
              device='none',
              push_id=''),
        users(account=auth_arr[5], phone=phones_arr[5],
              # friend_phones=[1, 2, 3, 4, 5],
              # friends=[1, 5],
              # followings=[2, 3, 4],
              # blocks=[],
              device='none',
              push_id=''),
        users(account=auth_arr[6], phone=phones_arr[6],
              # friend_phones=[1],
              # friends=[1],
              # followings=[],
              # blocks=[],
              device='none',
              push_id=''),
        users(account=auth_arr[7], phone=phones_arr[7],
              # friend_phones=[1],
              # friends=[1],
              # followings=[],
              # blocks=[],
              device='none',
              push_id=''),
        users(account=auth_arr[8], phone=phones_arr[8],
              # friend_phones=[],
              # friends=[],
              # followings=[],
              # blocks=[],
              device='none',
              push_id=''),
        users(account=auth_arr[9], phone=phones_arr[10],
              # friend_phones=[1, 2, 3, 4],
              # friends=[1, 2, 3],
              # followings=[],
              # blocks=[],
              device='none',
    ...
    users.objects.using(db_alias).bulk_create(users_arr)

    threads = apps.get_model('pine', 'Threads')
    threads.objects.using(db_alias).bulk_create([
        threads(author=1, is_public=False,
                readers=[1, 2, 3, 4, 5, 6, 7, 8],
                likes=[],
                max_like=0,
                reports=[],
                pub_date='2014-06-01T00:00:00+00:00',
                image_url='test_image.png',
                content='I\'m Namhoon. This threads can be viewed only my friends Sujin, Hanyong, etc.'),
        threads(author=1, is_public=True,
                readers=[1, 2, 3, 4, 5, 6, 7, 8],
                likes=[],
                max_like=0,
                reports=[],
                pub_date='2014-06-01T00:00:01+00:00',
                image_url='test_image.png',
                content='I\'m Namhoon. This threads can be viewed anyone.'),
        threads(author=6, is_public=False,
                readers=[1, 5, 6],
                likes=[],
                max_like=0,
                reports=[],
                pub_date='2014-06-01T00:04:00+00:00',
                image_url='test_image.png',
                content='I\'m pf. Choi. It\'s friends thread.'),
        threads(author=6, is_public=True,
                readers=[1, 5, 6],
                likes=[],
                max_like=0,
                reports=[],
                pub_date='2014-06-01T01:24:50+00:00',
                image_url='test_image.png',
                content='I\'m pf. Choi. It\'s public thread.'),
        threads(author=5, is_public=False,
                readers=[1, 5, 6],
                likes=[],
                max_like=0,
                reports=[],
                pub_date='2014-06-19T17:42:50+00:00',
                image_url='test_image.png',
                content='I\'m pf. Joo. Friends thread.'),
        threads(author=1, is_public=True,
                readers=[1, 2, 3, 4, 5, 6, 7 , 8],
                likes=[],
                max_like=0,
                reports=[],
                pub_date='2014-06-19T17:44:42+00:00',
                image_url='test_image.png',
                content='Namhoon. public. 한글 나랏말씀이 귕국에 달하시데'),
        threads(author=1, is_public=False,
                readers=[1, 2, 3, 4, 5, 6, 7, 8],
                likes=[2, 3, 4, 5, 6, 7, 8],
                max_like=0,
                reports=[],
                pub_date='2014-06-19T17:45:38+00:00',
                image_url='test_image.png',
                content='Namhoon. friends. ^^ Smile every day. ^.^ ^-^ !@#$%^&*()`_+|'),
    ])

    comments = apps.get_model('pine', 'Comments')
    comments.objects.using(db_alias).bulk_create([
        comments(author=1, thread=1,
                 likes=[1, 2, 3],
                 max_like=0,
                 reports=[4],
                 pub_date='2014-06-01T00:00:01+00:00',
                 content='Namhoon\'s first comment.'),
        comments(author=8, thread=1,
                 likes=[],
                 max_like=0,
                 reports=[],
                 pub_date='2014-06-01T00:00:02+00:00',
                 content='랄랄라 ^-^'),
        comments(author=4, thread=1,
                 likes=[],
                 max_like=0,
                 reports=[],
                 pub_date='2014-06-01T00:00:03+00:00',
                 content='hello'),
        comments(author=1, thread=8,
                 likes=[],
                 max_like=0,
                 reports=[],
                 pub_date='2014-06-19T17:45:39+00:00',
                 content='comment'),
    ])
